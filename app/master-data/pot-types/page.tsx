...
import { PotTypeManagementContent } from "@/components/pot-type-management-content";
import { TopNavbar } from "@/components/top-navbar";
import { SidebarInset, SidebarProvider } from "@/components/ui/sidebar";
import { getPotTypeRows } from "@/lib/pot-type-data";

export default async function PotTypesPage() {
  const potTypes = await getPotTypeRows();

  return (
    <SidebarProvider>
      <AppSidebar activePath="/master-data/pot-types" />
      <SidebarInset>
        <div className="flex flex-1 flex-col">
          <TopNavbar
            searchPlaceholder="Search potentiometer, value, taper, function..."
            items={[
          ...
          <PotTypeManagementContent initialPotTypes={potTypes} />
        </div>
      </SidebarInset>
    </SidebarProvider>
  );
