...
export default async function PublicCreatorPage(
  ...
  const items: MyDesignItem[] = [
    ...savedSetups.map((item) => ({
      ...
      editorHref: item.publishedTemplateId
        ? `/preview/${item.publishedTemplateId}`
        : "",
      targetId: item.id,
    ...
    ...componentDrafts.map((item) => ({
      ...
      publishedTemplateName: item.publishedComponentAssetId
        ? "Published asset"
        : null,
    })),
