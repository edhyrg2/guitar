...
export async function PUT(
  request: Request,
  context: RouteContext<"/api/pickup-models/[id]">
) {
  ...
  const pickupModel = await prisma.pickupModel.update({
    where: { id },
    data: {
      pickupBrandId,
      pickupTypeId,
  ...
  return NextResponse.json(mapPickupModelResponse(pickupModel));
}
...
export async function DELETE(
  ...
  await prisma.pickupModel.delete({
    where: { id },
  });
