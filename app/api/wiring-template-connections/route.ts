...
export async function GET() {
  ...
  return NextResponse.json(connections.map(mapConnectionResponse));
}
...
export async function POST(request: Request) {
  ...
  try {
    const connection = await prisma.wiringTemplateConnection.create({
      data: {
        wiringTemplateId,
        fromComponentRole,
        fromPointKey,
    ...
    return NextResponse.json(mapConnectionResponse(connection), { status: 201 });
  } catch (error) {
