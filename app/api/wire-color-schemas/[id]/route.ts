...
export async function PUT(
  request: Request,
  context: RouteContext<"/api/wire-color-schemas/[id]">
) {
  ...
  const schema = await prisma.wireColorSchema.update({
    where: { id },
    data: {
      pickupBrandId,
      name,
  ...
  return NextResponse.json(mapWireColorSchemaResponse(schema));
}
...
export async function DELETE(
  ...
  await prisma.wireColorSchema.delete({
    where: { id },
  });
